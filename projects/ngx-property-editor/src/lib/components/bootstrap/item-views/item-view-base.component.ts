...
export abstract class ItemViewBaseComponent implements OnChanges, AfterViewInit {
   ...
   * Wait with `updateItems` until that time because otherwise, it is called for each registered item
  ...
  public ngAfterViewInit(): void {
    ...
    // Update items once after all items have been registered
    this.updateItems();
  }
  ...
  public updateItems(): void {
    // Wait with `updateItems` until that time because otherwise, it is called for each registered item
    // instead of once in the end (by `ngAfterViewInit`).
    if (!this._afterViewInitDone) return;
